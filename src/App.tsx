...
export default function App(): React.JSX.Element {
  ...
  const renderEditor = (
    <div className="editor-area flex-1 min-h-0 flex flex-col">
  ...
  /* button style classes (cooler styles).
     NOTE: switched selected (filled) buttons to white with hover showing --bg-2.
     This satisfies "leave it white and hover should be color of the background" */
  const btnBase = "px-4 py-1 rounded-md text-sm font-semibold shadow-[0_8px_28px_rgba(0,0,0,0.12)] transform-gpu transition";
  const btnSelected = "bg-white text-[var(--text)] hover:bg-[color:var(--bg-2)] hover:scale-[1.02]"; // white, hover -> background color
  const btnUnselected = "bg-white text-[var(--text)]"; // neutral white button
  const btnDark = "bg-black text-white px-3 py-1 rounded-md text-sm font-semibold shadow-md hover:scale-105 transform transition"; // for import/export top buttons
...
document.querySelectorAll('a[href="#"]').forEach(a=>a.addEventListener('click', e=>e.preventDefault()));
  ...
  return (
    <div className="min-h-screen flex flex-col bg-[color:var(--bg-1)] text-[var(--text)]">
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex items-center justify-between h-14 gap-4">
            ...
            <div className="flex items-center gap-3">
              {/* refined logo badge using Tailwind */}
              <div
                className="w-11 h-11 rounded-lg flex items-center justify-center bg-gradient-to-b from-white/3 to-transparent ring-1 ring-white/5 shadow-xl"
                aria-hidden
              >
              ...
              <div className="leading-tight">
                ...
                <div className="text-xs text-white/60 -mt-0.5">Sandbox</div>
              </div>
            </div>

            {/* desktop controls - finer spacing and compact grouping
                REMOVED: the small utility "Swap/Reset" buttons from top navbar to declutter (per request).
            ...
            <div className="hidden md:flex items-center gap-4">
              ...
              {/* NEW: Open in Browser (ships current project to new tab immediately) */}
              <div>
                <button
                  onClick={() => openInBrowserFull()}
                  className="px-3 py-1 rounded-md bg-white text-[var(--text)] shadow-md hover:bg-[color:var(--bg-2)] transition"
      ...
      {/* hero + view controls (unchanged logic, but spacing/contrast improved) */}
      <header className="px-4 sm:px-6 lg:px-8 py-6 md:py-8">
        <div className="max-w-7xl mx-auto flex flex-col md:flex-row md:items-center md:justify-between gap-8">
          <div className="mb-2 md:mb-0">
            <h1 className="text-2xl md:text-3xl font-extrabold text-[var(--text)]">Build, run & learn, a sandbox for students</h1>
            <p className="mt-3 text-sm text-gray-700 max-w-xl">Learn by doing search help, package lessons, and share projects without leaving the editor. Your workspace is private to your device.</p>
          </div>
          ...
          <div className="flex items-center gap-4">
            <div className="inline-flex items-center gap-2 p-1 rounded-md bg-[color:var(--bg-2)] border border-gray-200">
              <button
                ...
                className={`${btnBase} ${viewMode === "split" ? btnSelected : btnUnselected} focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-white/20`}
              ...
              <button
                ...
                className={`${btnBase} ${viewMode === "editor-full" ? btnSelected : btnUnselected} focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-white/20`}
              ...
              <button
                ...
                className={`${btnBase} ${viewMode === "preview-full" ? btnSelected : btnUnselected} focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-white/20`}
            ...
            <button
              ...
              className={`px-3 py-1 rounded-md bg-white border text-[var(--text)] focus:outline-none focus:ring-2 focus:ring-white/10`}
            >
              Swap
            </button>
